...
const SettingIconButton = ({ toggleVisibility }) => (
...
const Button = ({ type, buttonText = '', toggleVisibility }) => {
  if (type === 'settings') {
    return <SettingIconButton toggleVisibility={toggleVisibility} />;
  }
  const buttonType = type === 'apply' ? 'submit' : 'button';

  return (
    <div className={`pane__${type}-row`}>
      <button type={buttonType} value={buttonText} className={`pane__${type}-preferences`} onClick={toggleVisibility}>
        {type === 'apply' ? 'Apply' : buttonText}
      </button>
    </div>
  );
};
